...
#
# Copyright (c) 2019-2020 Xuechun Hou
# Licensed under the Apache License, Version 2.0 (the "License");
# you may not use this file except in compliance with the License.
# You may obtain a copy of the License at
#
    ...
    try:
        while True:
            ...
            if (msg is not None):
                ...
                # example packet : {'X21': {'x': 703, 'radius': 10, 'y': 303, 'colour': '#55124f'}}
                #print("what is my packet", packet)
                for each in packet:
                    myWorld.set(each, packet[each])
...
def subscribe_socket(ws):
    ...
    # send client the current world 
    client.put(json.dumps(myWorld.world())) 
    try:
        while True:
            # block here
            msg = client.get()
            #print("what is the client messages ?", msg)
            ws.send(msg)
